...
TRANSLIT_WORDS = ["vostok", "imperia", "pobeda", "bereg", "kultura", "sovet", "otklik", "zvezda", "priboy", "vektor", "osnova"]
TRANSLIT_AFFIXES = ["space", "lab", "vibe", "club", "one", "media", "hub", "zone", "core", "hq"]
BRAND_ROOTS = ["aeth", "chron", "sol", "vort", "lum", "phos", "zeph", "celest", "aeg", "crest"]
BRAND_SUFFIXES = ["is", "ex", "or", "um", "on", "us", "ia", "ix", "ar", "al", "io", "ora"]
...
def calculate_catch_score(username: str, is_free: bool):
    if not is_free:
        return {"rank": "F", "status": "Занят пользователем", "color": "#ef4444", "score": 0}
    username = username.lower().replace("@", "").strip()
    if len(username) < 5 or any(c.isdigit() or c == "_" for c in username):
        return {"rank": "F", "status": "Содержит символы", "color": "#ef4444", "score": 0}

    length = len(username)
    if length == 5:
        return {"rank": "SSS+", "status": "Эксклюзив (~200-800+ TON)", "color": "#29c75f", "score": 100}
    if length == 6:
        return {"rank": "SS", "status": "Редкий ник (~50-200 TON)", "color": "#eab308", "score": 85}
    if length == 7:
        return {"rank": "S", "status": "Премиум бренд (~15-50 TON)", "color": "#a855f7", "score": 65}
    return {"rank": "A", "status": "Красивый ник (~3-15 TON)", "color": "#3b82f6", "score": 40}
                    ...
                    if is_free:
                        for chat_id, targets in list(db.items()):
                            if username in [t.lower() for t in targets]:
                                try:
                                    await bot.send_message(chat_id=int(chat_id), text=f"🚨 **РАДАР:** Ник `@{username}` свободен!", parse_mode="Markdown")
                                except Exception: pass
        except Exception: pass
...
async def get_game_state(user_id: str):
    ...
    # Расчет оффлайн-фарма (макс 3 часа = 10800 сек)
    ...
    return {
        ...
        "offline_seconds": min(time_diff, 10800) if offline_earned > 0 else 0
    }
...
async def sync_tap(request: Request):
    ...
    if taps > 0 and taps <= 50: # Защита от автокликера
    ...
    return {"status": "ok", "coins": user["coins"], "energy": user["energy"]}
...
# ... (Остальные роуты корневого генератора)
@app.get("/")
async def read_root():
...
async def generate_username(
    request: Request, platform: str = Query("telegram"), len_mode: str = Query("5-6"),
    mode: str = Query("standard"), user_input: str = Query(""), category: str = Query("auto_detect"),
    position: str = Query("mix"), finder_name: str = Query("Аноним"), finder_id: str = Query("0")
):
    for _ in range(10):
    ...
    fallback = generate_standard_brand(len_mode)
    is_free_fb = await check_username_telethon(fallback)
    eval_data = calculate_catch_score(fallback, is_free_fb)
    return {"status": "success", "generated_username": fallback, "platform": platform, "is_free": is_free_fb, "evaluation": eval_data}

@app.get("/api/leaderboard")
async def get_leaderboard(period: str = Query("all")):
    return {"status": "ok", "leaderboard": load_leaderboard()[:100]}
...
async def radar_list(chat_id: str): return {"targets": load_radar_db().get(str(chat_id).strip(), [])}
